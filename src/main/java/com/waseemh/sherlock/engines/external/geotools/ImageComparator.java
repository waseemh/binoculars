 ...
 * 
 * Note: Few modifications were inserted into this piece of code in order to
 * add support for exporting diff image after compare operation.
 * Date: Nov. 2014
 * 
 */
public class ImageComparator {
	...
	public void compare(ComparisonMode mode, RenderedImage expectedImage, RenderedImage actualImage) {
		...
		expectedImage = normalizeImage(expectedImage);
		actualImage = normalizeImage(actualImage);

		this.bands = expectedImage.getSampleModel().getNumBands();
		final boolean hasAlpha = expectedImage.getColorModel().hasAlpha();
		...
		computeDifference(expectedImage, actualImage);
		mismatchPercent = (mismatchCount * 1d / (width * actualImage.getHeight()) ) * 100;
	}
	...
	private void computeDifference(RenderedImage image1, RenderedImage image2) {
